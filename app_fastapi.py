...
# Ensure UTF-8 support in console
...
@app.get("/")
async def root():
    return {"message": "✅ BTC AI Dashboard backend is live and working."}
        ...
        for i in range(2, len(df)):
            ...
            if (not open_trade) and (trades_today < 2):
                ...
                if breakout and entry_price:
                    ...
                    if pd.isna(ema21) or pd.isna(rsi14):
                        continue

                    # allow entry if price is around EMA (±0.5%) and RSI not extreme
                    ema_tolerance = 0.005
                    price_vs_ema_ok = abs(entry_price - ema21) / ema21 <= ema_tolerance
                    if direction == "LONG":
                        if (entry_price < ema21 and not price_vs_ema_ok) or (rsi14 >= 78):
                            continue
                    else:  # SHORT
                        if (entry_price > ema21 and not price_vs_ema_ok) or (rsi14 <= 22):
                            continue
...
def strategy_projection(interval: str = "15m", limit: int = 500):
    ...
    try:
        ...
        for i in range(2, len(df) - 1):
            ...
            if is_prev_bear and is_curr_bull and row["close"] > prev["high"] and row["close"] > ema21 and rsi14 < 70:
                entry = row["close"]
